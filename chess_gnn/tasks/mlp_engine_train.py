...
from chess_gnn.models import ChessMLPEngine
...
@HydraConfigurable
class MLPEngineTrain(Task):
    def __init__(self, model: ChessMLPEngine, datamodule: ChessDataModule, trainer_factory: TrainerFactory, checkpoint_loader: Optional[CheckpointLoader] = None):
        super().__init__()
        seed_everything(42)
        ...
        ckpt_dir = Path('/home/ray/lightning_checkpoints/chess_mlp_engine') / self.uid
        trainer_factory.resolve_checkpoint_callback(ckpt_dir=str(ckpt_dir))
        trainer_factory.resolve_logger(project_name='chess-mlp-engine')
...
if __name__ == '__main__':
    config_path = get_config_path("MLPEngineTrain")
    task = MLPEngineTrain.from_hydra_configuration(LocalHydraConfiguration(str(config_path)))
    task.debug_run()
